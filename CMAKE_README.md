...
CMake will automatically detect whether you're using Homebrew or MacPorts and configure accordingly.
...
brew install cmake fftw opus libbsd iniparser \
    libusb portaudio ncurses pkgconfig
```
...
**Note:** The build system detects your package manager automatically:
- Apple Silicon Homebrew: `/opt/homebrew`
- Intel Homebrew: `/usr/local`
- MacPorts: `/opt/local`

If CMake can't find libraries, you can manually specify:
```bash
cmake .. -DCMAKE_PREFIX_PATH=/opt/homebrew  # or /opt/local for MacPorts
```
...
# Copy CMakeLists.txt to the root directory
...
### Component-Based Packaging (Recommended)

The build system uses **component-based packaging** to create separate packages for runtime and systemd support. This allows maximum compatibility across different init systems.

**Single build creates two packages:**
...
Creates:
- `ka9q-radio-1.0.0-Linux.deb` - Main runtime (no systemd dependency)
- `ka9q-radio-systemd-1.0.0-Linux.deb` - systemd support (optional)

**Install on systemd systems:**
```bash
sudo dpkg -i ka9q-radio_1.0.0_amd64.deb ka9q-radio-systemd_1.0.0_all.deb
```

**Install on non-systemd systems:**
```bash
...
### Linux without systemd (Devuan, sysvinit, runit, etc.)
The runtime package works perfectly on non-systemd systems:
```bash
# Install runtime package only
sudo dpkg -i ka9q-radio_1.0.0_amd64.deb

# Start radiod directly
/usr/local/sbin/radiod -v /etc/radio/radiod@hf.conf

# Or create init scripts for your init system (see INIT_SYSTEMS.md)
```

**Key advantage:** Unlike previous approaches, you can build on **any** system (systemd or not) and the resulting runtime package will work everywhere. The systemd support is in a separate optional package.

See **COMPONENTS.md** for full details on component-based packaging.
...
The build automatically detects and configures for:
- Homebrew on Apple Silicon (`/opt/homebrew`)
- Homebrew on Intel (`/usr/local`)
- MacPorts (`/opt/local`)

If you have both installed, Homebrew takes precedence. You can override:
```bash
cmake .. -DCMAKE_PREFIX_PATH=/opt/local  # Force MacPorts
```

**Missing Libraries:**
Some Linux-specific libraries (avahi) won't be available. The build
will skip features that require them - this is expected on macOS.
...
**On macOS:**
The build should auto-detect your package manager, but if libraries aren't found:
```bash

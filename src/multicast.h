...
static inline int setup_mcast_in(char const *source, struct sockaddr *source_sock, char const *target, struct sockaddr *sock, uint16_t offset, int tries){
...
static inline uint8_t *put16(uint8_t *dp,int x){
  assert(dp != NULL);
  *dp++ = (uint8_t)(x >> 8);
  *dp++ = (uint8_t)x;
  return dp;
